...
class UserBehaviorDao @Autowired constructor
    ...
    fun sqlExecuteQuery(conn: SQLConnection, mobile: String, taskId: String, moth: String): Single<ResultSet> {
        val sql = "select distinct\n" +
                ...
                "    cast(b.net_flow as SIGNED) as net_flow,\n" +
                "    cast((b.total_amount) as SIGNED) as total_amount,\n" +
                "    concat(substr('$moth',1,4),'-',substr('$moth',5)) as cell_mth,\n" +
                ...
                "    cast((select ifnull(sum(cv.duration_in_second),0) from carrier_voicecall cv where cv.mobile='$mobile' and cv.task_id='$taskId' and cv.bill_month='$moth' and cv.deleted_at=0) as SIGNED) as call_time,\n" +
                ...
                "    cast((select ifnull(sum(cv.duration_in_second),0) as dial_time from carrier_voicecall cv where cv.dial_type = 'DIAL' and cv.mobile='$mobile' and cv.task_id='$taskId' and cv.bill_month='$moth' and cv.deleted_at=0) as SIGNED) as dial_time,\n" +
                ...
                "    cast((select ifnull(sum(cv.duration_in_second),0) as dialed_time from carrier_voicecall cv where cv.dial_type = 'DIALED' and cv.mobile='$mobile' and cv.task_id='$taskId' and cv.bill_month='$moth' and cv.deleted_at=0) as SIGNED) as dialed_time,\n" +
                ...
                "    cast((select ifnull(sum(cr.amount_money),0) as rechange_amount  from carrier_recharge cr where cr.mobile='$mobile' and cr.task_id='$taskId' and DATE_FORMAT(date(cr.recharge_time),'%Y-%m')='$moth' and cr.deleted_at=0) as SIGNED) as rechange_amount\n" +
