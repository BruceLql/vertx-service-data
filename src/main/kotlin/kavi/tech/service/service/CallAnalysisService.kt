...
class CallAnalysisService {
    ...
    fun toCleaningCircleFriendsData(mobile: String, task_id: String): Single<JsonObject> {
        ...
        return this.client.rxGetConnection().flatMap { conn->
            ...
            jsonObject.put("avg_call_cnt_3m", if (it[3].rows.size==0) 0 else it[3].rows[0].getValue("avg_call_cnt_3m")) // 近3月平均通话次数
            jsonObject.put("avg_call_cnt_6m", if (it[4].rows.size==0) 0 else it[4].rows[0].getValue("avg_call_cnt_6m")) // 近6月平均通话次数
            ...
            jsonObject.put("avg_call_dialed_time_3m", if (it[28].rows.size==0) 0 else it[28].rows[0].getValue("avg_call_dialed_time_3m"))//近3月被叫月均通话时长
            jsonObject.put("avg_call_dialed_time_6m", if (it[29].rows.size==0) 0 else it[29].rows[0].getValue("avg_call_dialed_time_6m"))//近6月被叫月均通话时长
